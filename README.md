...
- **Уведомления администратору** о новых записях и отменах (необязательно).
...
| `ADMIN_CHAT_ID`           |     ❌      | ID чата админа для уведомлений (узнать у `@userinfobot`) |
...
Чтобы получать уведомления о новых записях и отменах:

1. Узнайте свой Telegram ID у бота [@userinfobot](https://t.me/userinfobot).
2. Укажите его в `.env`: `ADMIN_CHAT_ID=ваш_id`.
3. Напишите боту `/start` (чтобы он мог отправлять вам сообщения).

Если `ADMIN_CHAT_ID` пуст — уведомления просто отключены.
